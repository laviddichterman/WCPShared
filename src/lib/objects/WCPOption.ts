...
import { IMenu, OptionPlacement, WCPProduct, WCPOption, MODIFIER_LOCATION } from '../types';
...
export function IsOptionEnabled(option: WCPOption, product: WCPProduct, bake_count : [number, number], flavor_count : [number, number], location: OptionPlacement, menu: IMenu) {
  // TODO: needs to factor in disable data for time based disable
    ...
    const placement = GetPlacementFromMIDOID(product, option.mt._id, option.mo._id);
    ...
    return (!option.mo.enable_function || WFunctional.ProcessProductInstanceFunction(product, menu.product_instance_functions[option.mo.enable_function])) && has_room_on_left && has_room_on_right && passes_bake_diff_test;
}