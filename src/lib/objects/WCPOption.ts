...
export function IsOptionEnabled(option: WCPOption, product: WCPProduct, bake_count: [number, number], flavor_count: [number, number], location: OptionPlacement, productInstanceFunctions: MenuProductInstanceFunctions) {
  ...
  const placement = GetPlacementFromMIDOID(product, String(option.mt._id), String(option.mo._id));
  ...
  return (!option.mo.enable_function || WFunctional.ProcessProductInstanceFunction(product, productInstanceFunctions[String(option.mo.enable_function._id)])) && has_room_on_left && has_room_on_right && passes_bake_diff_test;
}