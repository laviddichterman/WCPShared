...
import { IProduct, IProductDisplayFlags, IProductInstance, OptionPlacement, ModifiersMap, MODIFIER_MATCH, MODIFIER_LOCATION, WCPProduct, IMenu, WProductMetadata } from '../types';
...
type LR_MODIFIER_MATCH_MATRIX = [MODIFIER_MATCH[][], MODIFIER_MATCH[][]];
...
export function WCPProductGenerateMetadata(a: WCPProduct, MENU : IMenu, service_time : moment.Moment) {
  ...
  const CheckMatchForSide = (side : MODIFIER_LOCATION, comparison, comparison_product) => {
    ...
    PRODUCT_CLASS.modifiers.forEach((pc_modifier, mtidx) => {
      ...
      for (let moidx = 0; moidx < CATALOG_MODIFIER_INFO.options_list.length; ++moidx) {
        ...
        const option_info = { 
          ...
          enable_left: is_enabled && option_object.mo.metadata.can_split && option_object.IsEnabled(product, OptionPlacement.LEFT, MENU),
          enable_right: is_enabled && option_object.mo.metadata.can_split && option_object.IsEnabled(product, OptionPlacement.RIGHT, MENU),
