...
type WInterval = [number, number]
type IntervalList = WInterval[];
interface ServicesEnableMap { [index:number]: boolean };

type OperatingHoursList = [IntervalList, IntervalList, IntervalList, IntervalList, IntervalList, IntervalList, IntervalList];
/**
 * @typedef {{blocked_off_union: IntervalList, operating_intervals: IntervalList, leadtime: Number, min_time_step: Number}} AVAILABILITY_INFO_MAP - an object containing...
 ...
 */
interface AvailabilityInfoMap { blocked_off_union: IntervalList, operating_intervals: IntervalList, leadtime: number, min_time_step: number };
export class WDateUtils {
  ...
  //  */  
  // static BlockedOffIntervalsForServicesAndDay(BLOCKED_OFF : {}, services : { [index:number]: boolean }, day : string) {
  //   let intervals = [];
  ...
  // /**
  //  * 
  //  * @param {IntervalList} intervals - array of length 2 arrays of Numbers
  //  * @returns {IntervalList} the input intervals array, sorted by interval start time, minimized to the union of the input array
  //  */
  // static ComputeUnionsForIntervals(intervals : IntervalList) {
  ...
  //   /**
  //  * 
  //  * @param {IntervalList} a - array of length 2 arrays of Numbers, sorted by start
  //  * @param {IntervalList} b - array of length 2 arrays of Numbers, sorted by start
  //  * @param {Number} step - the next available interval step resolution
  //  * @returns {IntervalList} a new array, the set subtraction of intervals a minus b
  //  */
  // static ComputeSubtractionOfIntervalSets(a : IntervalList, b : IntervalList, step : number) {
  ...
  //  */
  // static GetInfoMapForAvailabilityComputation(BLOCKED_OFF, SETTINGS, LEAD_TIMES, date, services, stuff_to_depreciate_map : { cart_based_lead_time: number; size: number; }) {
  ...
  // static AddIntervalToOperatingHours(service_index : number, day_index : number, interval, new_interval_map) {
