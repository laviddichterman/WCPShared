...
const DeepCopyPlacedOptions = (modifiers) => {
  const ret = {};
  Object.keys(modifiers).forEach(mid => {
    ret[mid] = modifiers[mid].map((x) => [x[0], x[1]]);
  });
  return ret;
}
...
export const CopyWCPProduct = (pi) => new WCPProduct(pi.PRODUCT_CLASS, pi.piid, pi.name, pi.description, pi.ordinal, pi.modifiers, pi.shortcode, pi.is_base, pi.display_flags);
...
export const WCPProduct = function(product_class, piid, name, description, ordinal, modifiers, shortcode, is_base, display_flags) {
  ...
  this.RecomputeMetadata = (MENU) => {
    ...
    Object.keys(this.modifiers).forEach(mtid => {
      this.modifiers[mtid].forEach((opt) => {
        ...
        is_split_compute = is_split_compute || opt[0] === TOPPING_LEFT || opt[0] === TOPPING_RIGHT;
      });
    });
  ...
  function Compare(first, other, MENU) {
    ...
    first.PRODUCT_CLASS.modifiers.forEach((modifier) => {
    ...
    first.PRODUCT_CLASS.modifiers.forEach((modifier, midx) => {
      const mtid = modifier.mtid;
      const first_option_list = Object.hasOwn(first.modifiers, mtid) ? first.modifiers[mtid] : [];
      const other_option_list = Object.hasOwn(other.modifiers, mtid) ? other.modifiers[mtid] : [];
      ...
      if (CATALOG_MODIFIER_INFO.modifier_type.min_selected === 1 && CATALOG_MODIFIER_INFO.modifier_type.max_selected === 1) {
        ...
        if (first_option_list.length === 1) {
          ...
          const other_option = other_option_list.length === 1 ? other_option_list[0] : "";
        ...
        CATALOG_MODIFIER_INFO.options_list.forEach((option, oidx) => {
          ...
          const first_option = first_option_list.find(val => val[1] == option.moid );
          // eslint-disable-next-line eqeqeq
          const other_option = other_option_list.find(val => val[1] == option.moid );
          const first_option_placement = first_option ? first_option[0] : TOPPING_NONE;
          const other_option_placement = other_option ? other_option[0] : TOPPING_NONE;
  ...
  this.Equals = (other, MENU) => {
    const comparison_info = Compare(this, other, MENU);
